...
def add_outliers(data, percentage_outliers=0.05):
    ...
    outlier_indices = np.random.choice(len(data), num_outliers, replace=False)# da b generate el indices bta3t el outliers (replace de 34an may3ml4 generate l nafs el index mareteen)
    # data_with_outliers = pd.Series(data.copy())
    data_with_outliers = data.copy()
    outliers = np.random.uniform(-1, 1, num_outliers)# da b generate random numbers been [-1, 1] b 3adad el outliers (w de el generated outliers)
    anomaly_mask = np.zeros(len(data_with_outliers), dtype=bool)# da mask array by2oly el outliers feen
    if len(outliers) > 0:
...
def main():
    ...
    date_rng = generate_time_series(start_date, start_date + timedelta(days=60), '12H')
    cyclic_component = add_cycles(date_rng, "exist", season_type="multiplicative")
    scaler = MinMaxScaler(feature_range=(-1, 1))
    # print(scaler.fit_transform(data_rng))
