...
class TestOutliersByPercentage(unittest.TestCase):
    ...
    def test_outliers_by_percentage_valid_percentage_success(self):
        ...
        outliers_by_percentage = OutliersByPercentage(self.data_component_values, percentage_outliers)
        self.assertIsInstance(outliers_by_percentage, OutliersByPercentage)

    def test_outliers_by_percentage_invalid_percentage_less_than_zero_failure(self):
        percentage_outliers = -0.25
        self.assertRaises(ValueError, OutliersByPercentage, self.data_component_values, percentage_outliers)
